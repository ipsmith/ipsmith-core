...
else
{
	...
	{
		echo "Role \"".$argv[1]."\" already exists.\n";
		exit(1);
	}
	else
	{
		$createRoleQuery = "INSERT INTO roles (name, createdby) VALUES (:name, :createdby);";
		$createRoleStmt  = $doctrineConnection->prepare($createRoleQuery);

		$createRoleStmt->bindValue('name',$argv[1]);
		$createRoleStmt->bindValue('createdby','commandline');

		$createRoleStmt->execute();

		echo "Role \"".$argv[1]."\" created.\n";
		exit(0);
	}
}